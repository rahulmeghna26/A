...
HITS_GROUP_ID = -1002506132292  # Replace with your group ID
APPROVED_GROUPS = set()
...
def load_admins():
    ...
    return set(["6521162324"])  # fallback default

def save_admins():
    with open(ADMINS_FILE, "w") as f:
        json.dump(list(ADMIN_IDS), f)

# Load approved groups from file on startup
if os.path.exists('approved_groups.txt'):
    with open('approved_groups.txt', 'r') as f:
        APPROVED_GROUPS.update(line.strip() for line in f.readlines())
...
def check_user_credits(user_id, required_credits):
    ...
    if user_id_str not in USER_CREDITS or USER_CREDITS[user_id_str]['date'] != str(today):
...
def get_remaining_credits(user_id):
    user_id_str = str(user_id)
    if user_id_str in USER_CREDITS:
        return USER_CREDITS[user_id_str]['credits']
...
<b>How to redeem</b>:
How to redeem: Use /redeem CODE in this chat 
Example: <code>/redeem {codes[0]}</code>
"""
...
def notify_on_restart():
    message = "✦ 𝑺𝒕𝒐𝒓𝒎 ✗ 𝘾𝙝𝙚𝙘𝙠𝙚𝙧 𖤐 Restarted Successfully.\nUse /ping to see my status."
...
"""
        ...
        # Optionally broadcast keys to all users and groups
        broadcast_redeem_codes(keys)
        
    except Exception as e:
        bot.reply_to(message, f"❌ Error: {str(e)}")

# Handle both /redeem and .redeem
...
def handle_redeem(message):
    try:
        ...
        # Mark key as used
        key_data['used'] = True
        key_data['used_by'] = str(message.from_user.id)
        key_data['used_date'] = datetime.now().strftime("%Y-%m-%d")
        ...
        USER_CREDITS[user_id]['credits'] += key_data['value']
        USER_CREDITS[user_id]['redeemed_credits'] = USER_CREDITS[user_id].get('redeemed_credits', 0) + key_data['value']
        
        # Check if user has reached 600 credits and upgrade to premium
        if USER_CREDITS[user_id]['redeemed_credits'] >= 600 and not is_user_subscribed(message.from_user.id):
...
✅ Redeemed {key_data['value']} credits!
🎉 Congratulations! You've been upgraded to Premium Plan for 30 days!
""")
        else:
            bot.reply_to(message, f"✅ Redeemed {key_data['value']} credits!")
        
        # Save redeem codes and user credits
        with open('redeem_codes.json', 'w') as f:
            json.dump(REDEEM_CODES, f)
...
# Handle both /broadcast and .broadcast
...
def handle_broadcast(message):
    ...
    try:
        ...
        if message.text.startswith('/'):
            ...
            broadcast_msg = message.text[10:].strip()  # remove ".broadcast "
            ...
            try:
                ...
                for user_id in all_users:
                    try:
                        ...
                        sent_msg = bot.send_message(user_id, broadcast_msg, parse_mode='HTML')
...
"""
                ...
                for group_id in all_groups:
                    try:
                        # Try to get chat info first to verify access
                        chat_info = bot.get_chat(group_id)
                        
                        # If we got chat info, try sending message
                        sent_msg = bot.send_message(group_id, broadcast_msg, parse_mode='HTML')
                        if sent_msg.message_id:
                            success += 1
                        else:
                            failed += 1
        ...
        if group_id in APPROVED_GROUPS:
            ...
            with open('approved_groups.txt', 'w') as f:
                for gid in APPROVED_GROUPS:
                    f.write(f"{gid}\n")
            bot.reply_to_message(message, f"✅ Group {group_id} removed from approved list!")
        else:
            bot.reply_to(message, f"❌ Group {group_id} is not approved.")
...
def handle_st(message):
    ...
    if not confirm_time():
        bot.reply_to(message, "❌ Checker is down. Follow @Darkboy336 for updates.")
        return
...
def list_admins(message):
    if str(message.from_user.id) != "6521162324":
...
✧ Name          :<code>{data.get('Unknown', 'N/A')}</code>
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                bot.send_photo(
                    ...
                    caption=(
                        f"✧ Image generated using Stability AI\n\n"
